...
\begin{figure}[H]
    ...
    \caption{Хистограма на разпределението на интервалите (Task 4)}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ybar,
            symbolic x coords={High Priority, Low Priority},
            xtick=data,
            ylabel={Средно време в опашка (Ave. Wait)},
            ymin=0,
            grid=major,
            width=0.8\linewidth,
            height=6cm,
            title={Сравнение на времето за чакане (Task 5a)}
        ]
        % Replace y-values with AVE.TIME from GPSS Queue statistics
        \addplot[fill=blue!50] coordinates {(High Priority, 10) (Low Priority, 50)};
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ybar,
            symbolic x coords={Processed, Dropped},
            xtick=data,
            ylabel={Брой транзакции (Transactions)},
            ymin=0,
            grid=major,
            width=0.8\linewidth,
            height=6cm,
            title={Обработени спрямо Отхвърлени заявки (Task 5b)}
        ]
        % Replace y-values with counts from GPSS (Total Entries vs Drop Block Entries)
        \addplot[fill=red!50] coordinates {(Processed, 800) (Dropped, 50)};
        \end{axis}
    \end{tikzpicture}
